...
def teardown():
    backup_filenames = [os.path.join(FIXTURE_PATH, filename) for filename in os.listdir(FIXTURE_PATH) if filename.endswith(".bak")]
...
def test_all_fixtures():
    ...
    fixture_in_files = [os.path.join(FIXTURE_PATH, fixture_file) for fixture_file in fixture_files if fixture_file.endswith("_in.py")]
    all_fixture_files = [(fixture_in, fixture_in.replace("_in.py", "_out.py")) for fixture_in in fixture_in_files]
...
def check_fixture(in_file, out_file):
    ...
    assert open(in_file).readlines() == open(out_file).readlines(), "Failure with %s" % in_file.split("/")[-1]
    print in_file
